(function initNjuGrabEngine(global) {
  ...
  const RANDOM_CADENCE_SENTINEL_MS = 30000; // 30000 档位现表示：每轮随机等待 1~3s
  ...
  function createGrabEngine(deps) {
    ...
    function calculateBackoffDelay(run, outcome, failures) {
      const outcomeBase = outcome === OUTCOME.RATE_LIMITED ? rateLimitBackoffBaseMs : transientBackoffBaseMs;
      const base = run.state.interval === RANDOM_CADENCE_SENTINEL_MS
        ? outcomeBase
        : Math.max(run.state.interval, outcomeBase);
    ...
    function scheduleNextRound(run, roundStartedAt) {
      ...
      let cadenceTarget = run.state.interval;
      if (cadenceTarget === RANDOM_CADENCE_SENTINEL_MS) {
        const rv = Number(random());
        cadenceTarget = 1000 + Math.floor((Number.isFinite(rv) ? Math.max(0, Math.min(1, rv)) : 0.5) * 2001);
      }
      const cadenceDelay = Math.max(minimumRestMs, cadenceTarget - elapsed);
    ...
    function start(taskValue, intervalMs, resumeSnapshot = null) {
      ...
      if (restoredRetryAt > now()) {
        ...
        log(run, resumeSnapshot
          ...
          : `🚀 开始监控，共 ${groups.length} 个课程组、${targets.length} 个目标，目标周期 ${
