...
function courseScopeNavigationPath(scope) {
  ...
  // GG01/GG02 carry an explicit parent; any other GGxx code is likewise a
  // public-course subcategory that lives under the 公共 (GG) tab, so navigate
  // through GG first before looking for the GGxx button.
  const parent = COURSE_SCOPE_INFO[normalized]?.parent
    || (normalized !== 'GG' && /^GG\d+$/.test(normalized) ? 'GG' : null);
...
let configuredGrabInterval = 3000;
let configuredIntervalMode = 'fixed';
let configuredIntervalMinMs = 3000;
let configuredIntervalMaxMs = 3000;
let grabTargetDecorateTimer = null;
let grabTargetObserver = null;
let grabPageStatusPanel = null;
let grabPageStatusTimer = null;
let grabPageCountdownTimer = null;
...
// runId for which the fast first-pass (try every missing scope once) plus the
// immediate follow-up round have already been performed.
let automaticCourseScopeWarmRunId = 0;
...
// ---- Page-side (选课页内面板) interval editor: fixed custom vs random range.
// Mirrors the popup control. Persisted into the same GRAB_TASK_CONFIG_KEY so
// both UIs stay in sync.

function syncConfiguredIntervalFromConfig(config) {
  const source = config && typeof config === 'object' ? config : {};
  const isRandom = source.intervalMode === 'random';
  configuredIntervalMode = isRandom ? 'random' : 'fixed';
  configuredGrabInterval = Math.max(1, Math.round(Number(source.intervalMs) || 0) || 3000);
  configuredIntervalMinMs = isRandom
    ? (Math.max(1, Math.round(Number(source.intervalMinMs) || 0)) || configuredGrabInterval)
    : configuredGrabInterval;
  configuredIntervalMaxMs = isRandom
    ? Math.max(configuredIntervalMinMs, Math.round(Number(source.intervalMaxMs) || configuredIntervalMinMs))
    : configuredGrabInterval;
}

function intervalClampMs(value, fallbackMs) {
  const parsed = Math.round(Number(value));
  if (!Number.isFinite(parsed) || parsed <= 0) return fallbackMs;
  return Math.min(600000, Math.max(1, parsed));
}

function readPageIntervalSpecFromEditor(editor, mode) {
  const customInput = editor.querySelector('[data-nju-grab-int-custom]');
  const minInput = editor.querySelector('[data-nju-grab-int-min]');
  const maxInput = editor.querySelector('[data-nju-grab-int-max]');
  if (mode === 'random') {
    const minMs = intervalClampMs(Math.round(Number(minInput?.value) || 0) * 1000, 3000);
    const maxMs = intervalClampMs(Math.round(Number(maxInput?.value) || 0) * 1000, 5000);
    const low = Math.min(minMs, maxMs);
    const high = Math.max(minMs, maxMs);
    return { mode: 'random', intervalMs: high, minMs: low, maxMs: high };
  }
  const customSeconds = Number(customInput?.value);
  const intervalMs = intervalClampMs(
    (Number.isFinite(customSeconds) && customSeconds > 0 ? customSeconds : 3) * 1000,
    3000
  );
  return { mode: 'fixed', intervalMs, minMs: intervalMs, maxMs: intervalMs };
}

function setPageIntervalModeView(editor, mode) {
  const isRandom = mode === 'random';
  editor.querySelectorAll('[data-nju-grab-int-mode]').forEach(button => {
    const on = button.getAttribute('data-nju-grab-int-mode') === mode;
    button.classList.toggle('is-on', on);
    if (button.getAttribute('role') === 'radio') button.setAttribute('aria-checked', on ? 'true' : 'false');
  });
  const fixedRow = editor.querySelector('[data-nju-grab-int-fixed]');
  const randomRow = editor.querySelector('[data-nju-grab-int-random]');
  if (fixedRow) fixedRow.hidden = isRandom;
  if (randomRow) randomRow.hidden = !isRandom;
}

function syncPageIntervalEditor(panel, source) {
  const editor = panel?.querySelector('[data-nju-grab-interval-editor]');
  if (!editor) return;
  const editable = isGrabPanelEditableStopped(source);
  const realRun = Boolean(
    source?.running
    || (source?.round || 0) > 0
    || (source?.configuredTargets?.length || source?.configuredGroups?.length || 0) > 0
  );
  const fromSource = realRun && source?.intervalMode;
  const mode = fromSource ? source.intervalMode : configuredIntervalMode;
  const isRandom = mode === 'random';
  const customMs = fromSource && !isRandom
    ? (Number(source.interval) || configuredGrabInterval)
    : configuredGrabInterval;
  const minMs = isRandom
    ? (fromSource ? (Number(source.intervalMinMs) || configuredIntervalMinMs) : configuredIntervalMinMs)
    : customMs;
  const maxMs = isRandom
    ? Math.max(minMs, fromSource ? (Number(source.intervalMaxMs) || minMs) : configuredIntervalMaxMs)
    : customMs;
  setPageIntervalModeView(editor, mode);
  const customInput = editor.querySelector('[data-nju-grab-int-custom]');
  const minInput = editor.querySelector('[data-nju-grab-int-min]');
  const maxInput = editor.querySelector('[data-nju-grab-int-max]');
  // Only fill the row of the active mode, so an untouched row keeps its own
  // defaults (fixed 3s / random 3~5s) until it is committed.
  if (isRandom) {
    if (minInput) minInput.value = String(Math.max(1, Math.round(minMs / 1000)));
    if (maxInput) maxInput.value = String(Math.max(1, Math.round(maxMs / 1000)));
  } else if (customInput) {
    customInput.value = String(Number((customMs / 1000).toFixed(3)));
  }
  editor.querySelectorAll('input,button').forEach(element => { element.disabled = !editable; });
}

function pageIntervalSummaryText(config) {
  const source = config && typeof config === 'object' ? config : {};
  if (source.intervalMode === 'random') {
    const min = Math.round(Number(source.intervalMinMs || 0) / 1000) || 1;
    const max = Math.max(min, Math.round(Number(source.intervalMaxMs || 0) / 1000) || min);
    return `随机 ${min}~${max} 秒`;
  }
  return `固定 ${Number((Number(source.intervalMs || 3000) / 1000).toFixed(3))} 秒`;
}

async function persistGrabIntervalSpec(intervalSpec) {
  if (!chrome.storage?.local) {
    syncConfiguredIntervalFromConfig({
      intervalMode: intervalSpec.mode,
      intervalMs: intervalSpec.intervalMs,
      intervalMinMs: intervalSpec.minMs,
      intervalMaxMs: intervalSpec.maxMs
    });
    return intervalSpec;
  }
  ...
  const next = grabTaskModel.normalizeTaskConfig({
    ...current,
    intervalMode: intervalSpec.mode,
    intervalMs: intervalSpec.intervalMs,
    intervalMinMs: intervalSpec.minMs,
  ...
  await chrome.storage.local.set({
    [GRAB_TASK_CONFIG_KEY]: { ...next, updatedAt: Date.now() },
    nju_grab_interval: String(next.intervalMs)
  });
  syncConfiguredIntervalFromConfig(next);
  return next;
}

function formatGrabInterval(intervalMs) {
  const interval = Math.max(1, Number(intervalMs) || 3000);
...
function ensureGrabPageStatusPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="nju-grab-status-body" data-nju-grab-status-body hidden>
       ...
       <div class="nju-grab-interval-editor" data-nju-grab-interval-editor>
         <span class="nju-grab-int-head">刷新间隔</span>
         <span class="nju-grab-int-mode" role="radiogroup" aria-label="刷新间隔模式">
           <button type="button" class="is-on" data-nju-grab-int-mode="fixed" role="radio" aria-checked="true">固定</button>
           <button type="button" data-nju-grab-int-mode="random" role="radio" aria-checked="false">随机</button>
         </span>
         <span class="nju-grab-int-row" data-nju-grab-int-fixed>
           <input type="number" data-nju-grab-int-custom min="0.001" max="600" step="0.001" value="3" aria-label="自定义间隔（秒）"><i>秒</i>
           <button type="button" data-nju-grab-int-apply-fixed>应用</button>
         </span>
         <span class="nju-grab-int-row" data-nju-grab-int-random hidden>
           <input type="number" data-nju-grab-int-min min="1" max="600" step="1" value="3" aria-label="随机最小间隔（秒）"><i>~</i>
           <input type="number" data-nju-grab-int-max min="1" max="600" step="1" value="5" aria-label="随机最大间隔（秒）"><i>秒</i>
           <button type="button" data-nju-grab-int-apply-random>应用</button>
         </span>
       </div>
  ...
  panel.querySelector('[data-nju-grab-control]')?.addEventListener('click', event => {
    ...
    if (isRunning || isGrabAuthRecoveryActive(currentState)) {
      ...
      chrome.storage.local.get([GRAB_TASK_CONFIG_KEY, 'nju_grab_courses', 'nju_grab_interval']).then(stored => {
        ...
        const interval = Number(stored.nju_grab_interval) || config.intervalMs || 3000;
  ...
  const intervalEditor = panel.querySelector('[data-nju-grab-interval-editor]');
  const intervalEditGuard = () => {
    const currentState = latestGrabPageState || getStateSnapshot();
  ...
  const commitPageInterval = async mode => {
    if (!intervalEditor) return;
    if (!intervalEditGuard()) return;
    const spec = readPageIntervalSpecFromEditor(intervalEditor, mode);
    try {
      const next = await persistGrabIntervalSpec(spec);
      syncPageIntervalEditor(panel, { running: false });
      showGrabPanelFeedback(`已保存监控间隔：${pageIntervalSummaryText(next)}`, 'success');
    } catch (error) {
  ...
  intervalEditor?.addEventListener('click', event => {
    event.preventDefault();
    event.stopPropagation();
    const modeButton = event.target.closest('[data-nju-grab-int-mode]');
    if (modeButton) {
      void commitPageInterval(modeButton.getAttribute('data-nju-grab-int-mode'));
      return;
    }
    if (event.target.closest('[data-nju-grab-int-apply-fixed]')) {
      void commitPageInterval('fixed');
      return;
    }
    if (event.target.closest('[data-nju-grab-int-apply-random]')) {
      void commitPageInterval('random');
      return;
    }
  });
  intervalEditor?.addEventListener('keydown', event => {
    if (event.key !== 'Enter') return;
    const isCustom = Boolean(event.target.closest('[data-nju-grab-int-custom]'));
    const isRandom = Boolean(event.target.closest('[data-nju-grab-int-min], [data-nju-grab-int-max]'));
    if (!isCustom && !isRandom) return;
    event.preventDefault();
    void commitPageInterval(isRandom ? 'random' : 'fixed');
  });
...
// When a course's own query slot passes, its countdown should restart from a
// full cycle immediately. Prefer an already-scheduled future stamp; otherwise
// derive it live from this round's start + the course's phase (roundStart +
// index*gap + k*interval), so the reset happens in real time rather than only
// when the whole round finishes.
function targetNextScanMs(source, targetState, currentTime) {
  const nextScanAt = Number(targetState?.nextScanAt || 0);
  if (nextScanAt > currentTime) return nextScanAt;
  const roundStart = Number(source.roundStartedAt) || 0;
  if (roundStart > 0) {
    const period = Math.max(1, Number(source.interval) || 3000);
    const offset = Math.max(0, Number(targetState?.scanOffsetMs) || 0);
    let next = roundStart + offset;
    while (next <= currentTime) next += period;
    return next;
  }
  return Number(source.nextRunAt) || nextScanAt;
}

function grabPageNextCheckText(source, currentTime) {
  ...
  const seconds = nextTimestamp > now ? Math.max(1, Math.ceil((nextTimestamp - now) / 1000)) : 0;
  const retryingAllTargets = Number(source.retryingTargetCount) > 0
    ...
    && Number(source.nextRetryAt) > now;
  return source.inFlight ? '正在检查…'
    : Number(source.globalRetryAt) > now ? `退避中 · ${seconds}s 后恢复`
      : retryingAllTargets ? `全体退避中 · ${seconds}s 后恢复`
      : source.phase === 'PAUSED_AUTH' ? '等待登录恢复'
        : seconds > 0 ? `下次检查：${seconds}s 后`
          : source.running ? '下次检查：准备中' : '下次检查：启动后检查';
}

// Lightweight "clock": while a run is active, refresh only the countdown/status
// text nodes every ~200ms instead of rebuilding the whole panel. The full
// render still runs on its own slower cadence to pick up real changes.
function refreshGrabPageCountdowns() {
  const panel = grabPageStatusPanel;
  const source = latestGrabPageState;
  if (!panel || !source || typeof source !== 'object') return;
  const currentTime = Date.now();
  const nextCheck = panel.querySelector('[data-nju-grab-next-check]');
  if (nextCheck) {
    const label = grabPageNextCheckText(source, currentTime);
    if (nextCheck.textContent !== label) nextCheck.textContent = label;
  }
  const rows = panel.querySelectorAll('.nju-grab-status-target[data-target-id]');
  for (const row of rows) {
    const status = row.querySelector('.nju-grab-target-status');
    if (!status) continue;
    const targetState = source.targetStates && typeof source.targetStates === 'object'
      ? (source.targetStates[row.dataset.targetId] || null)
      : null;
    const presentation = grabPageTargetPresentation(
      targetState,
      Boolean(source.running),
      { currentTime, nextRunAt: targetNextScanMs(source, targetState, currentTime) }
    );
    const text = presentation.detail || '';
    if (status.textContent !== text) status.textContent = text;
  }
}

function scheduleGrabPageCountdownRefresh() {
  if (grabPageCountdownTimer) clearTimeout(grabPageCountdownTimer);
  grabPageCountdownTimer = null;
  if (!latestGrabPageState?.running || !grabPageStatusPanel?.isConnected) return;
  grabPageCountdownTimer = setTimeout(() => {
    grabPageCountdownTimer = null;
    refreshGrabPageCountdowns();
    scheduleGrabPageCountdownRefresh();
  }, 200);
}
...
function renderGrabPageStatus(state) {
  ...
  syncPageIntervalEditor(panel, source);
  const nextCheck = panel.querySelector('[data-nju-grab-next-check]');
  if (nextCheck) {
    const label = grabPageNextCheckText(source, currentTime);
    if (nextCheck.textContent !== label) nextCheck.textContent = label;
...
async function initializeCourseTargetControls() {
  ...
  try {
    ...
    syncConfiguredIntervalFromConfig(config);
  } catch {
    configuredGrabTargetIds = new Set();
    configuredGrabTargets = [];
    configuredGrabGroups = [];
    syncConfiguredIntervalFromConfig(null);
  }
...
function activateCourseTabElement(element, isActive = () => true) {
  ...
  // Some course-scope tabs are <a href="javascript:..."> anchors. A synthetic
  // activation can make the browser follow that javascript: URL, which the
  // page's CSP blocks. Neutralize the anchor while we dispatch, so the site's
  // own click handler still switches the tab but no javascript: navigation runs.
  ...
  const href = anchor && typeof anchor.getAttribute === 'function'
    ...
    : null;
  const isJavaScriptHref = typeof href === 'string' && /^\s*javascript:/i.test(href);
  ...
  if (isJavaScriptHref && anchor && typeof anchor.addEventListener === 'function') {
    const suppressNavigation = event => event.preventDefault();
    anchor.setAttribute('href', '#');
    anchor.addEventListener('click', suppressNavigation, { capture: true });
    try {
      return dispatchActivation();
    } finally {
      anchor.removeEventListener('click', suppressNavigation, { capture: true });
...
function scheduleAutomaticCourseScopeNavigation(scopes) {
  ...
  const scope = candidates.find(candidate => {
    ...
    const currentRunId = Math.max(0, Number(latestGrabPageState?.runId) || 0);
    if (!(currentRunId > 0)) return false;
    // First pass of a run: try every missing scope once, back to back, without
    // waiting out the 4s throttle so scopes get established quickly at startup.
    if (lastRunId === undefined || lastRunId !== currentRunId) return true;
  ...
  scheduledTimer.timeoutId = setTimeout(() => {
    ...
    void jumpToCourseTab(scope, false, token).finally(() => {
      const isCurrent = token.generation === automaticCourseScopeNavigationGeneration;
      if (isCurrent) {
        automaticCourseScopeNavigationInFlight = false;
        automaticCourseScopeNavigationInFlightToken = null;
      }
      if (!isCurrent || !isAutomaticCourseScopeNavigationActive(token)) return;
      const stillMissing = missingCourseScopesForState(latestGrabPageState);
      if (stillMissing.size === 0) return;
      const currentRunId = Math.max(0, Number(latestGrabPageState?.runId) || 0);
      // Keep chaining the fast first pass until every currently-missing scope
      // has been attempted once this run (no waiting for the next cycle).
      let firstPassRemaining = false;
      for (const missingScope of stillMissing) {
        const attemptRunId = automaticCourseScopeLastAttemptRunId.get(normalizeCourseScope(missingScope));
        if (attemptRunId === undefined || attemptRunId !== currentRunId) {
          firstPassRemaining = true;
          break;
        }
      }
      if (firstPassRemaining) {
        setTimeout(() => {
          if (isAutomaticCourseScopeNavigationActive(token)) {
            scheduleAutomaticCourseScopeNavigation(missingCourseScopesForState(latestGrabPageState));
          }
        }, 250);
        return;
      }
      // Entire first pass finished: run one round now so the just-established
      // scopes are queried immediately instead of after a whole paced cycle.
      if (currentRunId > 0 && automaticCourseScopeWarmRunId !== currentRunId) {
        automaticCourseScopeWarmRunId = currentRunId;
        setTimeout(() => {
          if (isAutomaticCourseScopeNavigationActive(token)
            && Number(latestGrabPageState?.runId || 0) === currentRunId) {
            runNowGrab();
          }
        }, 500);
      }
