(function initNjuGrabCourseProvider(global) {
  ...
  // Lower bound for the space between two course-query requests in a round.
  // Never go below this even when interval/course-count asks for more.
  const MIN_QUERY_PACING_MS = 200;
  ...
  function createNetworkQueryClient(options = {}) {
    ...
    async function query(searches, context = {}) {
      ...
      // Spread the round's course queries evenly over one monitoring cycle:
      // with N courses and cycle `intervalMs`, send one request every
      // intervalMs / N ms so the server never sees an instant burst.
      const intervalMs = Math.max(0, Number(context.intervalMs) || 0);
      const pacingMs = safeSearches.length > 0 && intervalMs > 0
        ? Math.max(MIN_QUERY_PACING_MS, Math.round(intervalMs / safeSearches.length))
        : 0;
      ...
      return new Promise((resolve, reject) => {
        ...
        eventTarget.dispatchEvent(new CustomEvent(QUERY_EVENT, {
          detail: JSON.stringify({ action: 'query', requestId, searches: safeSearches, pacingMs })
        }));
      });
    }
