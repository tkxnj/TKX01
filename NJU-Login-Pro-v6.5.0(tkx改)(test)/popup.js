...
const els = {
  ...
  fixedIntervalRow: document.getElementById('fixedIntervalRow'),
  randomIntervalRow: document.getElementById('randomIntervalRow'),
  randomMinSeconds: document.getElementById('randomMinSeconds'),
  randomMaxSeconds: document.getElementById('randomMaxSeconds'),
  applyRandomInterval: document.getElementById('applyRandomInterval'),
...
// The committed interval setting (fixed value, or random min/max). Persisted
// writes always read this instead of the raw inputs, so half-edited numbers are
// not silently saved until the user presses one of the 应用 buttons.
function getIntervalSpecFromControls() {
  if (currentIntervalSpec) return currentIntervalSpec;
  const fixedMs = readCarrierIntervalMs() || 3000;
  return { mode: 'fixed', intervalMs: fixedMs, minMs: fixedMs, maxMs: fixedMs };
}
...
function getConfiguredTaskConfig() {
  ...
  const spec = getIntervalSpecFromControls();
  return grabTaskModel.normalizeTaskConfig({
    ...withKeywords,
    intervalMode: spec.mode,
    intervalMs: spec.intervalMs,
    intervalMinMs: spec.mode === 'random' ? spec.minMs : spec.intervalMs,
    intervalMaxMs: spec.mode === 'random' ? spec.maxMs : spec.intervalMs
  });
}
...
function renderGrabState() {
  ...
  const intervalMinMs = Number(grabState?.intervalMinMs) || 0;
  const intervalMaxMs = Number(grabState?.intervalMaxMs) || 0;
  const intervalText = grabState?.intervalMode === 'random' && intervalMinMs > 0
    ? `间隔 ${Number((intervalMinMs / 1000).toFixed(3))}~${Number((intervalMaxMs / 1000).toFixed(3))}s（随机）`
    : `间隔 ${Math.round(interval / 1000)}s`;
  ...
  if (!grabConnected) {
    ...
    if (globalRetryMs > 0) {
      ...
      els.grabSummarySub.textContent = retryingTargetCount > 0
        ...
        : `${progressText}${scanStatusText ? `；${scanStatusText}` : ''}；${intervalText}`;
    }
...
function normalizeGrabInterval(value, fallback = 3000) {
...
function randomIntervalLabel(minMs, maxMs) {
  const low = Math.min(minMs, maxMs);
  const high = Math.max(minMs, maxMs);
  return `${Number((low / 1000).toFixed(3))} ~ ${Number((high / 1000).toFixed(3))} 秒 · 随机`;
}

function intervalMode() {
  const active = els.intervalGrid.querySelector('.interval-option.active');
  return active?.dataset?.mode === 'random' ? 'random' : 'fixed';
}

function selectIntervalMode(mode) {
  const target = mode === 'random' ? 'random' : 'fixed';
  els.intervalGrid.querySelectorAll('.interval-option[data-mode]').forEach(btn => {
    const on = btn.dataset.mode === target;
    btn.classList.toggle('active', on);
    if (btn.getAttribute('role') === 'radio') btn.setAttribute('aria-checked', on ? 'true' : 'false');
  });
  els.fixedIntervalRow.hidden = target !== 'fixed';
  els.randomIntervalRow.hidden = target !== 'random';
}

function readIntervalInputMs(input, fallbackMs) {
  const raw = Number(input.value);
  if (!Number.isFinite(raw) || raw <= 0) return normalizeGrabInterval(fallbackMs);
  return normalizeGrabInterval(raw * 1000);
}

function readCarrierIntervalMs() {
  const parsed = Number(els.grabInterval.value);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
}

function setIntervalCarrier(value) {
  const interval = normalizeGrabInterval(value);
...
function setIntervalValue(value, { persist = true } = {}) {
  const interval = setIntervalCarrier(value);
  els.grabIntervalCustom.value = secondsText(interval);
  els.intervalLabel.textContent = formatGrabInterval(interval);
  els.intervalGrid.querySelectorAll('.interval-option[data-value]').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.value === String(interval));
  });
  if (persist) persistGrabTaskConfig();
  return interval;
}

function applyIntervalConfig(spec, { persist = true } = {}) {
  const isRandom = spec?.mode === 'random';
  // Random mode works in whole-second steps: bounds are snapped to a second
  // so the range shown matches exactly what the engine can sample.
  const quantizeSecond = (value, fallbackMs) => {
    const raw = Math.round(Number(value) / 1000) * 1000;
    return normalizeGrabInterval(Number.isFinite(raw) && raw > 0 ? raw : 0, fallbackMs);
  };
  const minMs = isRandom
    ? quantizeSecond(spec.minMs, 3000)
    : normalizeGrabInterval(spec.intervalMs, 3000);
  const maxMs = isRandom
    ? quantizeSecond(spec.maxMs, 5000)
    : minMs;
  const low = Math.min(minMs, maxMs);
  const high = Math.max(minMs, maxMs);
  currentIntervalSpec = {
    ...
    intervalMs: high,
    minMs: isRandom ? low : high,
    maxMs: high
  };
  selectIntervalMode(currentIntervalSpec.mode);
  // Each mode keeps its own inputs so switching never leaks one mode's value
  // into the other (e.g. random's upper bound must not become fixed's value).
  if (isRandom) {
    els.randomMinSeconds.value = secondsText(low);
    els.randomMaxSeconds.value = secondsText(high);
    els.intervalLabel.textContent = randomIntervalLabel(low, high);
  } else {
    els.grabIntervalCustom.value = secondsText(high);
    setIntervalValue(high, { persist: false });
  }
  if (persist) persistGrabTaskConfig();
  return currentIntervalSpec;
}

function applyFixedInterval() {
  selectIntervalMode('fixed');
  const ms = readIntervalInputMs(els.grabIntervalCustom, readCarrierIntervalMs() || 3000);
  const spec = applyIntervalConfig({ mode: 'fixed', intervalMs: ms });
  showToast(`刷新间隔已设为 ${formatGrabInterval(spec.intervalMs)}`);
  return spec;
}

function applyRandomInterval() {
  selectIntervalMode('random');
  const minMs = readIntervalInputMs(els.randomMinSeconds, 3000);
  const maxMs = readIntervalInputMs(els.randomMaxSeconds, 5000);
  const spec = applyIntervalConfig({
    mode: 'random',
    minMs: Math.min(minMs, maxMs),
    maxMs: Math.max(minMs, maxMs)
  });
  showToast(`随机间隔已设为 ${randomIntervalLabel(spec.minMs, spec.maxMs)}`);
  return spec;
}

function applyIntervalConfigFromConfig(config) {
  const source = config && typeof config === 'object' ? config : {};
  if (source.intervalMode === 'random') {
    applyIntervalConfig({
      mode: 'random',
      minMs: source.intervalMinMs,
      maxMs: source.intervalMaxMs
    }, { persist: false });
    return;
  }
  applyIntervalConfig({ mode: 'fixed', intervalMs: source.intervalMs || 3000 }, { persist: false });
}
...
function applyGrabSnapshot(state) {
  ...
  // Only adopt the engine's interval when the snapshot reflects a real (or
  // stopped/completed) run. A pristine idle snapshot carries interval:3000 and
  // would otherwise clobber the interval the user configured in the popup.
  const hasRealRun = Boolean(
    state?.running
    || (state?.round || 0) > 0
    || (state?.configuredTargets?.length || state?.configuredGroups?.length || 0) > 0
  );
  if (hasRealRun && state?.intervalMode === 'random') {
    applyIntervalConfig({
      mode: 'random',
      minMs: state.intervalMinMs,
      maxMs: state.intervalMaxMs
    }, { persist: false });
  } else if (hasRealRun && state?.interval) {
    applyIntervalConfig({ mode: 'fixed', intervalMs: state.interval }, { persist: false });
  }
  updateCourseCount({ persist: false });
  renderGrabState();
...
function initSettings() {
  chrome.storage.local.get(storageKeys, data => {
    ...
    renderCourseTags();
    applyIntervalConfigFromConfig(grabTaskConfig);
...
function initLoginEvents() {
  ...
  els.recognizeAgainBtn.addEventListener('click', async () => {
    if (captchaTestCapability.mode !== 'click-mark') return;
...
function initGrabEvents() {
  ...
  els.intervalGrid.addEventListener('click', event => {
    ...
    if (!btn) return;
    if (btn.dataset.mode) {
      // Switching mode commits using the numbers currently in the matching row.
      if (btn.dataset.mode === 'random') applyRandomInterval();
      else applyFixedInterval();
      return;
    }
    if (btn.dataset.value) setIntervalValue(btn.dataset.value);
  });

  els.applyGrabIntervalCustom.addEventListener('click', applyFixedInterval);
  ...
  els.applyRandomInterval.addEventListener('click', applyRandomInterval);
  [els.randomMinSeconds, els.randomMaxSeconds].forEach(input => {
  ...
  els.grabBtn.addEventListener('click', async () => {
    ...
    const interval = taskConfig.intervalMs || 3000;
