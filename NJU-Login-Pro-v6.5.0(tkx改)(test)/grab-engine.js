(function initNjuGrabEngine(global) {
  ...
  function normalizeTask(value, intervalMs) {
    ...
    return {
      intervalMs: Math.max(1, Number(intervalMs) || 3000),
      intervalMode: value?.intervalMode === 'random' ? 'random' : 'fixed',
      intervalMinMs: value?.intervalMinMs,
      intervalMaxMs: value?.intervalMaxMs,
      targets,
  ...
  function createGrabEngine(deps) {
    ...
    function snapshot(run = activeRun) {
      ...
      return {
        ...
        intervalMode: run.state.intervalMode || 'fixed',
        intervalMinMs: run.state.intervalMinMs ?? run.state.interval,
        intervalMaxMs: run.state.intervalMaxMs ?? run.state.interval,
        round: run.state.round,
        inFlight: run.state.inFlight,
        nextRunAt: run.state.nextRunAt,
        roundStartedAt: run.state.roundStartedAt,
    ...
    // Random mode re-samples the next cadence for every round so the pattern
    // never becomes a fixed cycle. Sampling is uniform over whole seconds in
    // [intervalMinMs, intervalMaxMs] (1s step). Fixed mode just returns the
    // configured interval.
    function pickIntervalMs(minValue, maxValue) {
      const lo = Math.max(1, Math.round(Number(minValue) || 1));
      const hi = Math.max(lo, Math.round(Number(maxValue) || lo));
      const loSec = Math.ceil(lo / 1000);
      const hiSec = Math.floor(hi / 1000);
      if (hiSec < loSec) {
        // Range narrower than one second: snap to the nearest whole second.
        return Math.max(1000, Math.round((lo + hi) / 2000) * 1000);
      }
      const second = loSec + Math.floor(random() * (hiSec - loSec + 1));
      return second * 1000;
    }

    function nextIntervalMs(run) {
      const state = run.state;
      if (state.intervalMode !== 'random') return state.interval;
      return pickIntervalMs(state.intervalMinMs, state.intervalMaxMs);
    }
    ...
    function scheduleNextRound(run, roundStartedAt) {
      ...
      const interval = nextIntervalMs(run);
      run.state.interval = interval;
      const cadenceDelay = Math.max(minimumRestMs, interval - elapsed);
      ...
      // Report an approximate per-course next-scan time so each pill shows its
      // own staggered countdown instead of a single shared "next round" value.
      // Anchor on this round's start: course i is queried at roundStart+i*gap in
      // each cycle, so its next occurrence is the smallest such instant after
      // now — which always stays within one interval (no "4s with a 3s cycle").
      const scanOrder = Array.isArray(run.scanOrder) ? run.scanOrder : [];
      const orderCount = Math.max(1, scanOrder.length);
      const scanGapMs = Math.max(1, interval) / orderCount;
      scanOrder.forEach((targetId, index) => {
        const targetState = run.targets.get(targetId);
        if (!targetState) return;
        const origin = roundStartedAt + index * scanGapMs;
        let nextScanAt = origin;
        while (nextScanAt <= currentTime) nextScanAt += interval;
        targetState.nextScanAt = nextScanAt;
      });
    ...
    async function runRound(run, options = {}) {
      ...
      // Keep the scan order and this round's start so the UI can drive each
      // course's own countdown live (reset at the moment that course is due),
      // instead of only updating at the end of the whole round.
      run.scanOrder = remaining.map(target => target.targetId);
      run.state.roundStartedAt = roundStartedAt;
      const pacedCount = Math.max(1, run.scanOrder.length);
      const pacedGapMs = Math.max(1, Number(run.state.interval) || 3000) / pacedCount;
      run.scanOrder.forEach((targetId, index) => {
        const targetState = run.targets.get(targetId);
        if (targetState) targetState.scanOffsetMs = index * pacedGapMs;
      });
    ...
    function start(taskValue, intervalMs, resumeSnapshot = null) {
      ...
      const interval = Math.max(1, Number(task.intervalMs) || 3000);
      const intervalMode = task.intervalMode === 'random' ? 'random' : 'fixed';
      let intervalMinMs = interval;
      let intervalMaxMs = interval;
      if (intervalMode === 'random') {
        const min = Math.round(Number(task.intervalMinMs));
        const max = Math.round(Number(task.intervalMaxMs));
        const lo = Number.isFinite(min) && min > 0 ? Math.min(600000, Math.max(1, min)) : interval;
        const hi = Number.isFinite(max) && max > 0 ? Math.min(600000, Math.max(1, max)) : lo;
        intervalMinMs = Math.min(lo, hi);
        intervalMaxMs = Math.max(lo, hi);
      }
      const initialInterval = intervalMode === 'random'
        ? pickIntervalMs(intervalMinMs, intervalMaxMs)
        : interval;
      ...
      if (restoredRetryAt > now()) {
        ...
        log(run, resumeSnapshot
          ...
          : `🚀 开始监控，共 ${groups.length} 个课程组、${targets.length} 个目标，目标周期 ${intervalMode === 'random' ? `${intervalMinMs / 1000}~${intervalMaxMs / 1000}s（每轮随机）` : `${interval / 1000}s`}`);
        void runRound(run);
      }
      return snapshot(run);
    }
