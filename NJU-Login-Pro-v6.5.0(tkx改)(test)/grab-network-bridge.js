...
(function installGrabNetworkBridge(global) {
  ...
  if (Xhr?.prototype?.open && Xhr?.prototype?.send) {
    ...
    global.document.addEventListener(COURSE_QUERY_EVENT, event => {
      ...
      const pacingMs = Math.max(0, Math.round(Number(detail.pacingMs) || 0));
      ...
      void (async () => {
        const results = [];
        const waitForPacing = ms => ms <= 0
          ? Promise.resolve(true)
          : new Promise(resolve => global.setTimeout(() => resolve(!control.cancelled), ms));
        let realReplays = 0;
        ...
        while (index < preparedSearches.length) {
          ...
          // Spread consecutive real course queries evenly over the monitoring
          // cycle: N courses at `intervalMs` => one request every interval/N ms,
          // so a single round never fires an instant burst at the server.
          if (realReplays > 0 && pacingMs > 0) {
            if (!await waitForPacing(pacingMs)) break;
          }
          index += 1;
