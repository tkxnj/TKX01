(function initNjuGrabTaskModel(global) {
  ...
  const INTERVAL_MAX_MS = 600000;

  function clampIntervalMs(value, fallback) {
    const parsed = Math.round(Number(value));
    if (!Number.isFinite(parsed) || parsed <= 0) return fallback;
    return Math.min(INTERVAL_MAX_MS, Math.max(INTERVAL_MIN_MS, parsed));
  }

  function normalizeInterval(value) {
    return clampIntervalMs(value, 3000);
  }

  // Random cadence uses an integer-ms range [minMs, maxMs]. Bounds are
  // clamped independently; if they arrive inverted the pair is swapped so the
  // effective range always has minMs <= maxMs.
  function normalizeIntervalRange(minValue, maxValue, fallbackMs) {
    const fallback = clampIntervalMs(fallbackMs, 3000);
    const min = clampIntervalMs(minValue, fallback);
    const max = clampIntervalMs(maxValue, fallback);
    return min <= max
      ? { minMs: min, maxMs: max }
      : { minMs: max, maxMs: min };
  }
  ...
  function normalizeTaskConfig(value, options = {}) {
    ...
    const intervalMs = normalizeInterval(source.intervalMs ?? options.intervalMs);
    const intervalMode = source.intervalMode === 'random' ? 'random' : 'fixed';
    // Fixed mode collapses the range onto intervalMs; random mode only trusts
    // explicit min/max bounds (falling back to intervalMs when absent).
    const range = normalizeIntervalRange(
      intervalMode === 'random' ? source.intervalMinMs : intervalMs,
      intervalMode === 'random' ? source.intervalMaxMs : intervalMs,
      intervalMs
    );
    return {
      ...
      intervalMs,
      intervalMode,
      intervalMinMs: range.minMs,
      intervalMaxMs: range.maxMs,
      groups,
